...
if [[ -n "$package" ]]; then
    ...
    pscp --user $ssh_user -h $unique_hosts $package $dst_home
    if [ $? -ne 0 ]; then
    ...
    pssh --user $ssh_user -h $unique_hosts "wget -O $package $benchbase_url"
    if [ $? -ne 0 ]; then
...
pssh --user $ssh_user -h $unique_hosts "tar -xzf `basename $package`"
if [ $? -ne 0 ]; then
