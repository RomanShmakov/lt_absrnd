...
kill_tpcc() {
    ...
    pssh -h $unique_hosts -i 'pkill -9 -f "^/bin/bash.*tpcc.sh"; pkill -9 -f "^java.*benchbase.jar -b tpcc"' &>/dev/null
    pssh -h $unique_hosts -i "cd $tpcc_path && rm -rf results_*" &>/dev/null
}
...
if ! which pssh >/dev/null; then
    echo "pssh not found, you should install pssh"
    exit 1
fi
...
pssh -h $hosts_file -i 'test -e $tpcc_script || (echo tpcc.sh does not exist && exit 1)'
if [ $? -ne 0 ]; then
...
if [[ -n "$ca_file_path" ]]; then
    ...
    pscp -h $unique_hosts $ca_file_path $tpcc_path/ &>/dev/null
    if [[ $? -ne 0 ]]; then
...
if [[ -z "$YDB_ANONYMOUS_CREDENTIALS" ]]; then
    if [[ -n "$token_file_path" ]]; then
        ...
        pscp -h $unique_hosts $token_file_path $tpcc_path/ &>/dev/null
        if [[ $? -ne 0 ]]; then
        ...
        pscp -h $unique_hosts $sa_key_file_path $tpcc_path/ &>/dev/null
        if [[ $? -ne 0 ]]; then
